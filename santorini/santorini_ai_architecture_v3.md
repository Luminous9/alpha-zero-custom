...
V3 also applies an explicit symmetry-consistency regularizer. By default, 25% of each optimizer batch is evaluated in a second, different D4 orientation during the same forward pass. The second policy is mapped back to the first orientation and trained to agree through a symmetric Jensen-Shannon loss; the two value predictions receive a mean-squared agreement loss. The CLI defaults remain `0.05` for backward-compatible experiments, while the current continuation run raises both weights to `0.10` and retains the paired fraction at `0.25`. These terms are added to the unchanged supervised policy cross-entropy and outcome-value losses. Pairing 25% of the batch adds approximately 25% to the network examples evaluated per optimizer step but does not increase the step budget, replay size, or checkpoint size. It is compatible with existing V3 checkpoints and replay files.
...
Training and evaluation use separate exploration scales. `--gumbel-scale` and `--gumbel-placement-scale` control self-play. A per-game placement mixture is available through `--placement-scale-exploration-probability` and `--placement-exploration-gumbel-scale`; one scale is selected when each self-play game starts and applies to all four of that game's placement turns. The current setup uses placement scale `1.5` for 90% of games and `2.25` for 10%, while standard play remains at `1.0`. `--evaluation-gumbel-scale` and `--evaluation-gumbel-placement-scale` override milestone and fixed-anchor arenas, so the mixture never changes evaluation. Evaluation uses scales `0.0` standard / `1.0` placement. Thus standard strength telemetry is deterministic while empty-board telemetry retains reproducibly varied placement searches.

Each iteration records the configured mixture, actual base and exploratory game counts, exact and D4-symmetry-unique completed opening counts, and the frequency of the most common exact opening and symmetry family. These fields are written to JSONL and TensorBoard, plotted by the Kaggle notebook, and summarized in the live log. They verify both that the intended 90/10 sampling occurred and whether the additional exploration produces meaningful opening diversity.
...
| Symmetry consistency | 25% paired draws; current policy/value weights 0.10/0.10 (CLI defaults 0.05/0.05) |
