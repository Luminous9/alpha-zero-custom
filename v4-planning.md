...
- **Bootstrap-to-self-play optimizer transition:** the first P100 arms applied
  ...
  Therefore no reuse ramp is currently authorized. A subsequent local
  diagnostic finds substantial exact target/prior KL and a 38-2 search96 win
  over the same P1c raw prior, ruling out a thin 96-simulation policy teacher.
  Repeated-state outcome variance is high, while iteration 8 fits `z` better
  but drifts sharply away from cached 250K-node values. The next step is a
  multi-arm fine-tuning diagnostic: retain fixed 2x at 3e-4 only as control;
  compare global 1e-4, a conservative-trunk/differential-head arm, and global
...
- The initial abrupt transition to self-play `z` was rejected by the controlled
  P2 fine-tuning diagnostic. At identical global 1e-4 LR, pure-`z` Arm B scored
  20.0% placement-inclusive against iteration 1, while Arm D's explicit frozen
  P1c-value bridge scored 77.5%; D also led standard play 65.0% to 52.5% and had
  less frozen-objective drift. The selected main-head exception therefore uses
  `v_target = (1-beta) * v_P1c(s) + beta * z`, with beta 0.25 at iteration 2
  and a linear absolute-iteration ramp to 1.0 at iteration 11. Preserve the
  frozen anchor digest and target-mode/beta telemetry. Iteration 11 is a
  mandatory review boundary before any post-bridge pure-`z` block.
...
- Placement-inclusive arenas have two declared estimands. The raw
  policy-weighted view preserves the multiplicity of naturally sampled learned
  openings. The diversity-controlled view hashes exact and D4 completed
  placements, caps any D4 family's normalized score contribution at 5%, and
  reports opening-frequency ESS, maximum multiplicity/share, and exact/full
  trajectory counts. Execute an exact deterministic continuation only once and
  reuse its result for repeated occurrences; do not add standard-move noise to
  manufacture different games. Promotion additionally requires raw and capped
  directions to agree and resolve, natural D4 ESS of at least 75% of the pair
  count, and no natural D4 family above 5%. A 40/100/120-game arena therefore
  permits at most 2/5/6 occurrences of one D4 family before controlled
  down-weighting. When the natural gate fails, collect a separate frozen suite
  of 40-60 D4-unique learned-policy openings and play both contestant seats on
  every opening.
...
8. **P2 - self-play (iteration 17 is the provisional full-game checkpoint; iteration 11 is the general-standard rollback):** the trainable canonical 6x192 wrapper, checkpoint migration, transformed-search preflight, replay/source metadata, paired reset-per-game sparring loop, frozen seam telemetry, and P100 throughput smoke pass. Exact live-policy calibration selects 5k ladder v2. Two reuse-ramp continuations from the accepted iteration-1 transition were discarded; the second reached iteration 8 but accumulated frozen objective from 0.75174 to 0.97853 and lost 9-31 standard to iteration 1. Local diagnostics then showed that 96-search retains strong policy teaching signal (38-2 over the same raw P1c prior), while single-game `z` is high variance and the failed iteration 8 moved away from the cached deep-value proxy while fitting `z`. The controlled fixed-2x fine-tuning arms selected D: global 1e-4 plus a frozen P1c-value-to-`z` bridge. D iteration 4 moved the teacher objective only +0.00654 and beat iteration 1 by 26-14 standard and 31-9 placement-inclusive. The continuation completed beta=1 at iteration 11 with cumulative objective only +0.02234 and no safety triggers. A six-iteration 4x-dose branch through iteration 17 also remained healthy, including frozen deep-value and prior-target-KL controls. Its legacy full-game score over iteration 11 was inflated by a learned placement distribution whose dominant D4 family occupied more than 40% of games. The fresh duplicate-aware decomposition executes each exact deterministic continuation once, reports raw and normalized-5%-capped views, and gates on D4 ESS. It finds iteration-17 placement plus iteration-11 standard at parity with iteration 11 (50.8% raw, 49.6% capped), while iteration 17's standard controller supplies a strong but not diversity-resolved edge on iteration-17-preferred openings (83.3% raw, 63.9% capped; natural D4 ESS 4.48 versus 45 required). A frozen suite of 60 D4-unique iteration-17-style openings then gives iteration 17 a 67-53 (55.8%) edge, with 19/29/12 paired sweeps/splits/losses and a 46.7-65.0% paired interval; every standard trajectory is unique and physical sides split 61-59. A replay audit then located the concentration mechanism: P1c/iteration-1/iteration-17 matched samples had D4 placement ESS 29.21/13.75/5.48, while standard replay remained 89.2% D4-unique across all 17 windows. Placement occupied 27.7% of iteration-17 stored self-play positions and its repeated root targets were nearly redundant (mean JS 0.0111). Raw replay is now preserved while the optimizer view aggregates identical D4 placement states per window/ply, averages targets, assigns 15% total placement mass equally across four plies, and uses square-root frequency weighting. Separately, 10% of ordinary games use search-generated D4-unique neural starts with no added move noise. The next matched four-iteration diagnostic from iteration 17 compares this full design to a zero/zero control before promotion. Use iteration 17 for complete games from the empty board, while retaining iteration 11 as the accepted general-standard anchor and rollback. Do not escalate to 6x/8x on this unresolved margin. Preserve flat snapshots, the single-ZIP Kaggle workflow, the standing prior-target KL watch, and the +0.05 step/+0.10 cumulative controls. Disagreement starts and the low-weight auxiliary head remain separately gated as specified in §6.2-6.3; Run13 remains a manual anchor only.
